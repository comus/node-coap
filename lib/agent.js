...
var util            = require('util')
  ...
  , dgram           = require('dgram')
  , parse           = require('./packet').parse
  , generate        = require('./packet').generate
...
function Agent(opts) {
  ...
  console.log('new Agent', 'called', opts)
...
Agent.prototype._init = function initSock(socket) {
  ...
  this._sock.on('message', function(msg, rsinfo) {
    console.log('this._sock on message!!!!')
    var packet
      , message
      , outSocket
  ...
  console.log('this._lastToken', this._lastToken)
  this._lastMessageId = Math.floor(Math.random() * (maxMessageId - 1))
  console.log('this._lastMessageId', this._lastMessageId)
...
Agent.prototype._handle = function handle(msg, rsinfo, outSocket) {
  ...
  console.log('block2Buff', block2Buff)
  var block2
  ...
  if (block2) {
    ...
    if (block2.moreBlock2) {
      ...
      // next block2 request
      console.log('createBlock2 called!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
...
Agent.prototype._nextMessageId = function nextToken() {
  console.log('_nextMessageId', 'called')
  if (++this._lastMessageId === maxMessageId) {
    this._lastMessageId = 1
  }

  console.log('_nextMessageId', 'this._lastMessageId', this._lastMessageId)
  console.log('_nextMessageId', 'maxMessageId', maxMessageId)
...
Agent.prototype.request = function request(url) {
  ...
  req = new OutgoingMessage({}, function(req, packet) {
    ...
    if (!(packet.ack || packet.reset)) {
      ...
      console.log('packet.token', packet.token)
...
function urlPropertyToPacketOption(url, req, property, option, separator) {
  if (url[property]) {
    const value = url[property].normalize('NFC').split(separator)
