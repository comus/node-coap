...
module.exports.generate = function generate(packet) {
  ...
  // first byte
  byte = 0
  console.log('[generate]', 'byte1', byte.toString(2).padStart(8, '0'))
  byte |= 1 << 6 // first two bits are version
  console.log('[generate]', 'byte2', byte.toString(2).padStart(8, '0'))
  byte |= confirmableAckResetMask(packet)
  console.log('[generate]', 'byte3', byte.toString(2).padStart(8, '0'))
  byte |= packet.token.length
  console.log('[generate]', 'byte4', byte.toString(2).padStart(8, '0'))
  buffer.writeUInt8(byte, pos++)
  console.log('[generate]', 'buffer', buffer.toString('hex'))

  console.log('[generate]', 'packet.code:', packet.code, ', codes[packet.code]:', codes[packet.code])
  ...
  console.log('[generate]', 'packet.messageId', packet.messageId, packet.messageId.toString(16))
  ...
  console.log('[generate]', 'packet.token', packet.token)
  ...
  console.log('[generate]', 'buffer (0)', buffer.toString('hex'))

  if (packet.code !== '0.00' && packet.payload != '') {

    // payload separator
    buffer.writeUInt8(255, pos++)
    console.log('[generate]', 'buffer (1)', buffer.toString('hex'))
    packet.payload.copy(buffer, pos)
    console.log('[generate]', 'buffer (2)', buffer.toString('hex'))
  }
...
function prepareOptions(packet) {
  ...
  for (i = 0; i < packet.options.length; i++) {
    ...
    console.log('packet.options[i].name', 'byte', byte, byte.toString(16))
    ...
    console.log('packet.options[i].name(final)', 'byte', byte, byte.toString(16))
